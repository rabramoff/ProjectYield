...
```{r switches}
do_maps = T
lowcolor = "#440154FF" #"#FF0000"
midcolor = "#21908CFF" #"#FF8000"
highcolor = "#FDE725FF" #"#FFFF00"
#viridis low="#440154FF", mid="#21908CFF", high="#FDE725FF"
##working - back to viridis but maybe change the second set of plots the difference between them
```
...
#make gridded maps
  ...
  gg1 <- ggplot() +
    ...
    print(gg1 + geom_tile(data=PrChangeData, aes(x=Longitude, y=Latitude, fill=Pr_avg), alpha=0.8) + scale_fill_gradient2(midpoint=0, low=lowcolor, mid=midcolor, high=highcolor, name="Avg Precip (mm)") + #, limits=c(-100,100)) + 
    ...
    print(gg1 + geom_tile(data=PrChangeData, aes(x=Longitude, y=Latitude, fill=Tave), alpha=0.8) + scale_fill_gradient2(midpoint=0, low=lowcolor, mid=midcolor, high=highcolor, name="Avg Temp (degC)") + #, limits=c(-100,100)) + 
...
map_df$Variable <- factor(map_df$Variable, 
    ...
    print(gg1 + geom_tile(data=map_df, aes(x=x, y=y, fill=Effect), alpha=0.8) + scale_fill_gradient2(midpoint=0, low=lowcolor, mid=midcolor, high=highcolor, name="Shapley Value") + #, limits=c(-100,100)) + 
...
###Plot####
    ...
    print(gg1 + geom_tile(data=map_df_all, aes(x=Longitude, y=Latitude, fill=Effect), alpha=0.8) + scale_fill_gradient2(midpoint=0, low=lowcolor, mid=midcolor, high=highcolor, limits=c(-100,100), name="Yield Change (%)") + 
    ...
    print(gg1 + geom_tile(data=map_change, aes(x=Longitude, y=Latitude, fill=Effect), alpha=0.8) + scale_fill_gradient2(midpoint=0, low=lowcolor, mid=midcolor, high=highcolor, limits=c(-100,100), name="Yield Change (%)") + 
      ggtitle(paste0("RCP 4.5")) +
