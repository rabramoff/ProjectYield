...
####18/02/2021####
#Developed further by Rose Abramoff - starting 22/02/2021
```
#Load libraries
```{r load-libraries}
library(openxlsx)
...
#TEMP: explore temp and precip
```{r}
#plot(TAB$`Current.Average.Temperature.(dC)_area_weighted`, Effect)
#plot(TAB$`Current.Average.Temperature_point_coordinate.(dC)`,Effect)

plot(TAB$Local.delta.T.from.2005, Effect) #may be useful
plot(TAB$Global.delta.T.from.2005, Effect)

#plot(TAB$`Current.Annual.Precipitation.(mm)._area_weighted`, Effect)
#plot(TAB$`Current.Annual.Precipitation.(mm)._point_coordinate`, Effect)

#Change in precip was estimated from current precip and the other stuff, so not sure it is useful to consider changes but could try it...
plot(TAB$`Base.precipitation.(annual).(mm).(local.base.period)`, Effect)
plot(TAB$`Base.precipitation.(seasonal).(mm).(local.base.period)`, Effect)
plot(TAB$`Annual.Precipitation.change.each.study.(mm)`, Effect)
plot(TAB$`Annual.Precipitation.change.from.2005.(mm)`, Effect)
plot(TAB$`Annual.Preciptation.change.(%).(relative.to.local.base)`, Effect)
plot(TAB$`Seasonal.Precipitation.change.(mm).each.study.(local.baseperiod)`, Effect)
```
...
if(adaptation_types){
    if(same_locations){
      ...
      DATA<-data.frame(Effect,Species,Delta_temp,CO2.ppm,TempAvg,Precip, Delta_precip, Future, Latitude, Longitude,Fertiliser,Irrigation,Cultivar,Soil_organic_matter_management,Planting_time,Tillage,Others)
}else{
    if(same_locations){
      Loca<-paste(Longitude,Latitude, sep="_") 
...
#samples
for(i in 1:length(crop_species)){
Locs[[i]]<-paste(df[[i]]$Longitude,df[[i]]$Latitude, sep="_")
...
dev.off()
```
#STOPPED HERE ADDING IN PRECIP, then do Yield Loop and Shap Loop
