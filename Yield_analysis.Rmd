...
pdf(file=paste0(crop_species[[i]],"_samelocs",same_locations,"_adaptypes",adaptation_types,"_ClimateRF.pdf"), height=4, width=4)
plot(Testing_1[[i]]$Effect,Pred_rf_0[[i]], ylab="Predicted Yield Change (%)", xlab = "Observed Yield Change (%)")
title(paste0(crop_species[[i]], ": Model 3"), line = 0.5)
dev.off()
...
pdf(file=paste0(crop_species[[i]],"_samelocs",same_locations,"_adaptypes",adaptation_types,"_ClimateLongLatRF.pdf"), height=4, width=4)
plot(Testing_1[[i]]$Effect,Pred_rf_1[[i]], ylab="Predicted Yield Change (%)", xlab = "Observed Yield Change (%)")
title(paste0(crop_species[[i]], ": Model 5"), line = 0.5)
dev.off()
...
pdf(file=paste0(crop_species[[i]],"_samelocs",same_locations,"_adaptypes",adaptation_types,"_ClimateLongLatGBM.pdf"), height=4, width=4)
plot(Testing_1[[i]]$Effect,Pred_gbm_1[[i]], ylab="Predicted Yield Change (%)", xlab = "Observed Yield Change (%)")
title(paste0(crop_species[[i]], ": Model 6"), line = 0.5)
dev.off()
...
```{r}
load("saved_MLs_samelocsFALSE_adaptationtypesFALSE_store_predictions.Rdata")
...
load(paste0("saved_MLs_samelocsFALSE_adaptationtypes",adaptation_types,"_store_PDPs.Rdata"))
```
...
soybean.effs <- bindeffs(PDPeffs, 4)
  ...
  #dplyr::summarise(mean.value = mean(.value), se = sd(.value)/sqrt(n()), n = n())
     rename(mean.value = .value)
     
      efdf.mean <- maize.effs %>% 
       dplyr::full_join(rice.effs) %>%
       dplyr::full_join(wheat.effs) %>%
       dplyr::full_join(soybean.effs) %>%
        group_by(.borders, .feature, .crop) %>%
  dplyr::summarise(mean.value = mean(.value), se = sd(.value)/sqrt(n()), n = n())
     ...
     efdf.mean$.crop <- as.factor(efdf.mean$.crop)
     
if(adaptation_types){
    efdf.adaptation <- filter(efdf.mean, .feature != "TempAvg" && .feature != "CO2.ppm" && .feature != "Delta_temp" && .feature != "Latitude" && .feature != "Longitude")
    ...
    efdf.adaptation <- efdf.mean[efdf.mean$.feature=="Adaptation",]
    efdf.others <- efdf[efdf$.feature!="Adaptation",]
     ...
     efdf.Longitude <- efdf.others[efdf.others$.feature=="Longitude",]
      ...
      ylabs = "Yield Change (%)"
      ylims = range(efdf$mean.value)+range(efdf$mean.value)*0.1
      
      ggplot_pdps <- function(efdf, xlabels) {
      ggplot(efdf, aes(.borders, mean.value, color= .crop)) +
        geom_point(alpha=0.3) + geom_smooth(se=TRUE, method="loess", span=0.75) + #geom_line() +
          #geom_errorbar(aes(ymin=mean.value-se, ymax=mean.value+se), width=0, position=position_dodge(.9)) +
        ylim(ylims) +
        ylab(ylabs) +
        xlab(xlabels) + 
        theme_bw() + theme(legend.position = "none") +
        scale_color_manual(values = c(cbPalette[1], cbPalette[2], cbPalette[3], cbPalette[4]), labels = c("Maize", "Rice", "Soybean", "Wheat")) + 
...
if(adaptation_types){
  p6 <- ggplot(efdf.adaptation, aes(x = .borders, y = mean.value, fill=.crop)) +
          geom_bar(stat = "identity") +
                #geom_errorbar(aes(ymin=mean.value-se, ymax=mean.value+se), width=.2, position=position_dodge(.9)) +
          facet_grid(.~.feature) +
          ylim(ylims) +
          ylab(ylabs) +
          xlab("") + 
          theme_bw() + theme(legend.position = "none", strip.text.x = element_text(size = 5)) +
  ...
  p7 <- ggplot(efdf.adaptation, aes(x = .borders, y = mean.value, fill=.crop)) +
          geom_bar(stat = "identity") + 
                #geom_errorbar(aes(ymin=mean.value-se, ymax=mean.value+se), width=.2, position=position_dodge(.9)) +
          ylim(ylims) +
          ylab(ylabs) +
